...
public:
    ...
    static void advanced(float t, float fadein_sec, float static_sec, float fadeout_sec, std::function<void(float r, Phase phase)> draw_fn);
    ...
    static void delta(float t, float fadein_sec, float static_sec, float fadeout_sec, T delta, std::function<void(T, float, float)> draw_fn){
        advanced(t, fadein_sec, static_sec, fadeout_sec, [&draw_fn, &delta](float r, Phase phase){
            if(phase == Phase::FADEIN){
                ...
                draw_fn((1.0f - r) * delta, (1.0f - r) * 255.0f, (1.0f - r));
            }
        });
    }
